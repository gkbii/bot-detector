...
// `--render` writes today's band table into EVALUATION.md between the markers
// below; `--check-render` only reports that it is stale, which is what
// test/evaluation.test.js calls. The 2026-08-05 table at the top of that
// document is a dated record of a live run and is NOT touched by either -- see
// the block comment at renderCurrentBands().
const RENDER = argv.includes('--render');
const CHECK_RENDER = argv.includes('--check-render');

const BAND_ORDER = ['low', 'moderate', 'high', 'insufficient-data'];

const EVALUATION_PATH = new URL('../EVALUATION.md', import.meta.url);
const BEGIN_MARKER = '<!-- BEGIN GENERATED: current-bands -->';
const END_MARKER = '<!-- END GENERATED: current-bands -->';
...
if (RENDER || CHECK_RENDER) {
  const current = fs.readFileSync(EVALUATION_PATH, 'utf8');
  const wanted = spliceEvaluation(current, renderCurrentBands());
  if (current === wanted) {
    console.log('\nEVALUATION.md: generated band table is up to date');
  } else if (CHECK_RENDER) {
    console.error('\nEVALUATION.md: generated band table is STALE -- run `npm run evaluate -- --render`');
    process.exit(1);
  } else {
    fs.writeFileSync(EVALUATION_PATH, wanted);
    console.log('\nEVALUATION.md: rewrote the generated band table');
  }
}
...
/**
 * Today's band table, as markdown.
 *
 * WHY ONLY THIS ONE BLOCK OF EVALUATION.md IS GENERATED. That document is a
 * dated log of findings: a live run against a real thread, then every defect
 * found since, each with its measurement, the decision it forced and a "what
 * this does not establish" section. None of that is script output -- five of
 * the eight `npm run measure:*` scripts go to the network, and regenerating
 * the document would both delete the reasoning and re-fetch a volunteer-run
 * free archive on every render. What IS a current measurement is this table,
 * and it comes from the frozen corpus with no network at all. So the dated
 * tables stay verbatim, and the one claim about TODAY is generated and held
 * fresh by a test.
 */
function renderCurrentBands() {
  const out = [];
  out.push(BEGIN_MARKER);
  out.push('<!-- Generated by `npm run evaluate -- --render`. Do not edit by hand. -->');
  out.push('');
  out.push(`**Today, off the frozen corpus** — ${threadHumans.length} thread humans + ${prolificHumans.length} prolific humans + ${bots.length} declared bots,`);
  out.push(`captured ${capturedAt.length ? new Date(Math.max(...capturedAt) * 1000).toISOString().slice(0, 10) : 'unknown'}, no network.`);
  out.push('');
  out.push(`| | ${AXES.join(' | ')} |`);
  out.push(`| --- | ${AXES.map(() => '---').join(' | ')} |`);
  for (const [label, group] of rows) {
    out.push(`| ${label} | ${AXES.map((axis) => cell(group, axis).trim()).join(' | ')} |`);
  }
  out.push('');
  out.push(`Separation on automation — no human above \`low\`, no bot at \`low\`: **${separated ? 'HOLDS' : 'BROKEN'}**`);
  out.push('');
  out.push(END_MARKER);
  return out.join('\n');
}

/** EVALUATION.md with the generated block replaced. Throws if the markers are gone. */
function spliceEvaluation(current, block) {
  const start = current.indexOf(BEGIN_MARKER);
  const end = current.indexOf(END_MARKER);
  if (start === -1 || end === -1) {
    throw new Error(
      `EVALUATION.md is missing the ${BEGIN_MARKER} / ${END_MARKER} markers -- ` +
        'restore them rather than removing the generated table'
    );
  }
