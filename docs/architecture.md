...
bot-detector/
  extension/                 the product — MV3, load-unpacked, no build step
    ...
    errors.js                the ONE error taxonomy — coded classes, shared by BOTH halves
    log.js                   log(domain, event, fields) — one line per event, greppable
    lib/sources/
  ...
  server/                    OPTIONAL. Adds a Claude read + a shared cache. Nothing else.
                             All .ts, on Node's native type stripping — no build step.
    index.ts                 node:http; dispatches through registry/routes.ts
    config.ts                reads through registry/env.ts and nothing else
    types.ts                 the server's view of AccountProfile and Verdict — NOT the core's schema
    deterministic.ts         lazily imports the EXTENSION's fetch + scoring. One implementation.
    pack.ts                  the evidence pack: which comments the model may talk about
    agenda.ts                the Claude call, with citation verification
    cache.ts                 node:sqlite — profiles / verdicts / LLM reads, three TTLs
    username.ts              normalisation as a security boundary (this value enters a URL)
    registry/                ENV, ROUTES, EVENTS — load-bearing data, not documentation
      env.ts                 every variable this repo reads; config.ts reads through it
      routes.ts              every route; index.ts dispatches through it
      events.ts              every worker message; was a comment block in background.js
      index.ts               the four registries plus COMMANDS, read out of package.json
  scripts/                   NOT shipped, NOT imported by anything, NOT run by npm test
    ...
    measure-band-edge.mjs       fetches; the before/after JIO-329 sweep — EVALUATION.md 4b
    ...
    render-registry.js       GENERATES docs/registry.md and .env.example from server/registry/
    typecheck.js             fetches tsc with npx; loud skip when it cannot — decision 0012
  ...
  test/                      the shared core's suite
    conformance.test.cjs     the fleet code standard, as a test — copied verbatim, never edited
    registry.test.js         ties every registry entry back to the code it describes
  ...
  docs/                      the diagram and feature list this project's public page is generated from
    ...
    registry.md              GENERATED by `npm run registry` -- do not hand-edit
    decisions/               one file per decision still in force and not testable
    size-exceptions.json     files over the 800-line ratchet; paths only, no comment key
```
